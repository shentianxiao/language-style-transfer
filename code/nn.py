...
def gumbel_softmax(logits, gamma, eps=1e-20):
    U = tf.random.uniform(tf.shape(input=logits))
...
def argmax_word(dropout, proj_W, proj_b, embedding):
    ...
    def loop_func(output):
        output = tf.nn.dropout(output, 1 - (dropout))
        logits = tf.matmul(output, proj_W) + proj_b
        word = tf.argmax(input=logits, axis=1)
        inp = tf.nn.embedding_lookup(params=embedding, ids=word)
        return inp, logits
    ...
    with tf.compat.v1.variable_scope(scope) as vs:
        ...
        for size in filter_sizes:
            with tf.compat.v1.variable_scope('conv-maxpool-%s' % size):
                ...
                pooled = tf.reduce_max(input_tensor=h, axis=1)
...
def discriminator(x_real, x_fake, ones, zeros,
    ...
    if wgan:
        eps = tf.random.uniform([], 0.0, 1.0)
        mix = eps * x_real + (1-eps) * x_fake
        d_mix = cnn(mix, filter_sizes, n_filters, dropout, scope, reuse=True)
        grad = tf.gradients(ys=d_mix, xs=mix)[0]
        grad_norm = tf.sqrt(tf.reduce_sum(input_tensor=tf.square(grad), axis=[1, 2]))
        loss = d_fake-d_real + eta*tf.square(grad_norm-1)
        return tf.reduce_mean(input_tensor=loss), -tf.reduce_mean(input_tensor=loss)

    else:
        loss_d = tf.reduce_mean(input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(
            labels=ones, logits=d_real)) + \
                 tf.reduce_mean(input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(
            labels=zeros, logits=d_fake))
        loss_g = tf.reduce_mean(input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(
